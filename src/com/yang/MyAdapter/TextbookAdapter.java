...
public class TextbookAdapter extends BaseAdapter {
	...
	public TextbookAdapter(Context context, List<Map<String, Object>> datas,
			int i) {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = listContainer.inflate(
					R.layout.activity_textbooklist_item, null);
			// 获取控件对象
			...
			listItemView.grade = (TextView) convertView
					.findViewById(R.id.grade);
			listItemView.course = (TextView) convertView
		...
		// 设置文字和图片
		listItemView.city.setText((String) datas.get(position).get("city"));
		listItemView.version.setText((String) datas.get(position)
				.get("version"));
		...
		if (position == i && i != -1) {
			...
			TextView courseTextView = (TextView) convertView
					.findViewById(R.id.course);
