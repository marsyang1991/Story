...
public class RegisterActivity extends Activity implements OnClickListener,
	...
	class CountDownHanlder extends Handler {
		...
		public void handleMessage(Message msg) {
			Button bt = (Button) RegisterActivity.this
					.findViewById(R.id.register_getSMSbt);
		...
		case R.id.register_okbt:
			if (cb.isChecked()) {
				...
				if (passwordString.trim().length() < 6
						|| passwordString.trim().length() > 20) {
					Toast.makeText(RegisterActivity.this, "密码长度为6-20",
							Toast.LENGTH_SHORT).show();
					return;
				}
				...
				Toast.makeText(RegisterActivity.this, "请接受使用条款和隐私政策",
						Toast.LENGTH_SHORT).show();
			}
			break;
		}
	...
	private boolean registerUserToMyDB() {
		...
		http.send(HttpMethod.GET, urlBase + "api_register.do?" + "userName="
				...
				new RequestCallBack<String>() {
					...
					public void onSuccess(ResponseInfo<String> responseInfo) {
						...
						JSONObject jsonObject = (JSONObject) jsonParser
								.nextValue();
						result = Boolean.parseBoolean(jsonObject
								.getString("rst"));
						if (result) {
