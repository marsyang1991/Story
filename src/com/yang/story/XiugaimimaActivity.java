...
public class XiugaimimaActivity extends Activity{
	...
	private void init()
	{
		...
		xiugai_wancheng.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View arg0) {
				//String yanzhengmaString = xiugai_yanzhengma.getText().toString();
				String pwString = xiugai_pw.getText().toString();
				String urlBase = getResources().getString(R.string.db_url_base);
					
				RequestParams params = new RequestParams();
				HttpUtils http = new HttpUtils();
				http.send(HttpMethod.GET, urlBase+"api_update_user.do?"+"userName="+dianhua+"&passwd="+pwString, null, new RequestCallBack<String>() {
					...
					public void onSuccess(ResponseInfo<String> responseInfo) {
						...
						boolean result = Boolean.parseBoolean(jsonObject.getString("rst"));
						if(result)
						{
							Toast.makeText(XiugaimimaActivity.this, "修改成功,请返回登录", Toast.LENGTH_SHORT).show();
							SharedPreferences preferences = getSharedPreferences("userInfo", MODE_PRIVATE);
							Editor editor = preferences.edit();
							editor.putBoolean("flag", false);
							startActivity(new Intent(XiugaimimaActivity.this,FrameActivity.class));
							finish();
						}
						else {
							Toast.makeText(XiugaimimaActivity.this, "修改失败", Toast.LENGTH_SHORT).show();
						}
