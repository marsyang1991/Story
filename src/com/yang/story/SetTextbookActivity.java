...
public class SetTextbookActivity extends Activity{
    ...
    private Textbook getSttedTextbook() {
		SharedPreferences sharedPreferences = getSharedPreferences("textbook", Context.MODE_PRIVATE);
    ...
    private void init()
    {
    	...
    	if(settedTextbook!=null)
    	{
    		currentCity = settedTextbook.getCity();
    		currentCourse = settedTextbook.getCourse();
    		currentGrade = settedTextbook.getGrade();
    		currentVersion = settedTextbook.getVersion();
    	}*/
    	...
    	skipTextView.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				startActivity(new Intent(SetTextbookActivity.this,FrameActivity.class));
    	...
    	textbookList.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				for(int i=0;i<arg0.getCount();i++)
				{
					...
					LinearLayout ll =(LinearLayout)view.findViewById(R.id.textbookListItemLayout);
					ll.setBackgroundColor(Color.parseColor("#ffffff"));
					TextView cityTextView = (TextView)view.findViewById(R.id.city);
					TextView courseTextView = (TextView)view.findViewById(R.id.course);
				...
				LinearLayout ll =(LinearLayout)arg1.findViewById(R.id.textbookListItemLayout);
				ll.setBackgroundColor(Color.parseColor("#F8E0CD"));
				TextView cityTextView = (TextView)arg1.findViewById(R.id.city);
				TextView courseTextView = (TextView)arg1.findViewById(R.id.course);
				TextView gradeTextView = (TextView)arg1.findViewById(R.id.grade);
				TextView versionTextView = (TextView)arg1.findViewById(R.id.version);
    ...
    private void updataToDb() {
		...
		http.send(HttpMethod.GET, urlBase+"api_update_user.do?"+data, null, new RequestCallBack<String>() {
			...
			public void onSuccess(ResponseInfo<String> responseInfo) {
				...
				if(flag)
				{
					Toast.makeText(SetTextbookActivity.this, "设置成功", Toast.LENGTH_SHORT).show();
					finish();
				}
				else {
					setResult(RESULT_CANCELED);
					Toast.makeText(SetTextbookActivity.this, "设置失败", Toast.LENGTH_SHORT).show();
				}
			}
			@Override
			public void onFailure(HttpException error, String msg) {
				Toast.makeText(SetTextbookActivity.this, "服务器错误", Toast.LENGTH_SHORT).show();
			}
			@Override
	...
	public void onChanged(WheelView wheel, int oldValue, int newValue) {
		if (wheel == cityWheelView) {
			updateGrade();
		} else if (wheel == gradeWheelView) {
			updateVersion();
		}else if(wheel==versionWheelView)
		{
			updateCourse();
		}
		else if (wheel == courseWheelView) {
			currentCourse = courseDatas.get(currentVersion)[newValue];
		}
	...
	private void updateGrade()
	{
		...
		currentGrade = gradeDatas.get(currentCity)[pCurrent];
		String[] grades = gradeDatas.get(currentGrade);
		
		if (grades == null) {
			grades = new String[] { "" };
		}
		gradeWheelView.setViewAdapter(new ArrayWheelAdapter<String>(this, grades));
		gradeWheelView.setCurrentItem(0);
		updateVersion();
	}
	private void updateVersion()
	{
		int pCurrent = gradeWheelView.getCurrentItem();
		currentGrade = versionDatas.get(currentCity)[pCurrent];
		String[] versions = gradeDatas.get(currentGrade);
		
		if (versions == null) {
			versions = new String[] { "" };
		}
		versionWheelView.setViewAdapter(new ArrayWheelAdapter<String>(this, versions));
		versionWheelView.setCurrentItem(0);
		updateCourse();
	}
	private void updateCourse()
	{
		int pCurrent = versionWheelView.getCurrentItem();
		currentVersion = versionDatas.get(currentGrade)[pCurrent];
		String[] courses = versionDatas.get(currentVersion);
		
		if (courses == null) {
			courses = new String[] { "" };
		}
		courseWheelView.setViewAdapter(new ArrayWheelAdapter<String>(this, courses));
		courseWheelView.setCurrentItem(0);
	}*/
